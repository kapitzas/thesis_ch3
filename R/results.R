...
# File paths
figure_path <- "/Users/simon/ownCloud/PhD/writing/thesis/chapters/figures/chapter3/"
...
boundary <- readOGR(file.path(".", "data", "bnd_ama", "Lim_Biogeografico.shp")) # Study aarea boundary feature
...
fig3bc <- plot_grid(fig3b, fig3c, nrow = 1, labels = c("(b)", "(c)"), vjust = c(0.2, 0.2), hjust = c(-1.1, -1.1), label_size = 10, label_fontface = "plain", rel_widths = c(2.5, 2))

pdf(file.path(figure_path, "fig3.pdf"), height = 4.5, width = medium_width)
plot_grid(fig3a, fig3bc, nrow = 2, labels = c("(a)", ""), vjust = c(1.5, -1), rel_heights = c(3,2), label_size = 10, label_fontface = "plain")
dev.off()
...
pdf(file.path(figure_path, "fig4.pdf"), height = 6, width = medium_width)

plot_grid(mp[[1]], mp[[2]], ncol = 1, legends_fig4, nrow = 3, rel_heights = c(1, 1, 0.5), labels = c("cropland", "pasture"), label_size = 10, vjust = c(24, 24), hjust = c(-3.3, -4))
...
# Fig 2. a
# South America
sa <- readRDS("/Volumes/external/OneDrive - The University of Melbourne/PhD - Large Files/raw data/Global/countries_shp/countries_gadm36_sp.rds")
sa <- sa[which(sa$UNREGION1 == "South America"),]
...
# Add protected areas
pa <- raster(file.path(".", "data", "data_ama","temp", "PA1992_ama.tif"))
pa[which(pa[] == 1)] <- NA
pa <- gplot_data(pa)
...
demands <- readRDS(file.path(".", "data", "data_ama", "demands.rds"))
...
pdf(file.path(figure_path, "fig2.pdf"), height = 3, width = single_width)
plot_grid(fig2a, fig2b, ncol = 2, labels = c("(a)", "(b)"), label_size = 10, label_fontface = "plain")
dev.off()
